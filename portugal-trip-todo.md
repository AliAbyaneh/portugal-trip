# Portugal Cycling Trip — TODO

**Trip date:** April 3, 2026 | **Days until departure:** ~30

## 🔴 Must Do
- [ ] **Book accommodation** for each night (8 nights)
  - [ ] Ovar (Apr 3)
  - [ ] Costa Nova / Aveiro (Apr 4)
  - [ ] Figueira da Foz (Apr 5)
  - [ ] Nazaré (Apr 6)
  - [ ] Peniche (Apr 7 + Apr 8 rest day — 2 nights)
  - [ ] Ericeira (Apr 9)
  - [ ] Lisbon (Apr 10)
- [ ] **Call Air Transat** — bike transport policy, fees, box requirements
- [ ] **Call Air Canada** — bike transport policy, fees, box requirements
- [ ] **Test ride the bike** — full gear loaded, 50-60 km test day
- [ ] **Get bike box/bag** for flight
- [ ] **Figure out the map** — GPX routes onto Garmin Epix, offline maps

## 🟡 Should Do
- [ ] Gear & equipment check (see packing list in trip doc)
- [ ] Buy any missing gear
- [ ] Download GPX routes to Garmin Epix
- [ ] Get Portugal SIM card plan sorted (or check roaming)
- [ ] Travel insurance (covers cycling?)
- [ ] Bike tune-up / service before trip

## 🟢 Nice To Have
- [ ] Research restaurants at each stop
- [ ] Learn basic Portuguese phrases
- [ ] Download offline maps (Google Maps / OsmAnd)
- [ ] Strava route builder — build each stage with heatmaps

---
*Last updated: March 4, 2026*

## Flight Details
- **Outbound:** Air Transat TS 764
  - Depart: Wed Apr 1, 2026 — 20:10 from YYZ (Terminal 3)
  - Arrive: Thu Apr 2, 2026 — 08:05 at OPO (Porto)
  - Aircraft: A330-200
  - Class: Economy (Eco Budget)
  - Non-stop

- **Return:** Air Canada AC 811
  - Depart: Tue Apr 14, 2026 — 13:00 from LIS (Terminal 1)
  - Arrive: Tue Apr 14, 2026 — 16:15 at YYZ (Terminal 1)
  - Aircraft: A330-300
  - Class: Economy (S) — Aeroplan Standard Reward
  - Non-stop, 8h 15m
